...
export async function addNewRoom(image, roomType, roomPrice) {
  ...
  try {
    const res = await axios.post("/api/rooms/addNewRoom", formData, {
      headers: getHeader(), // Use getHeader function to get the correct headers
    });
...
export async function getAvailableRooms(checkIn, checkOut, roomType) {
  const result = await api.get(
    `/api/rooms/available-rooms?checkIn=${checkIn}&checkOut=${checkOut}&roomType=${roomType}`
  );
  return result;
}
